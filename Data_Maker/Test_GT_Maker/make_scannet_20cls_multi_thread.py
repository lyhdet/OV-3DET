...
nyu40ids_semseg = np.array(
    [33,4,5,6,17,7,3,10,18,34,36,24,14,32,12,8,16,29,35,37]
)
...
def get_3d_bbox(xyzrgb):
    ...
    for ind, cur_end in enumerate(step_idx):
        ...
        if cluster_ind.shape[0]<=100:
            print("insufficient points 1")
            cur_start = cur_end
            continue

        cur_xyz = cur_xyz[cluster_ind,:]
        cur_bbox_3d = np.zeros(8)

        #print(cur_xyz.shape)

        cur_bbox_3d[:7] = compute_bbox(cur_xyz)[0,:7]
        cur_bbox_3d[7] = nyu40id2class_semseg[cur_cls]
        print(nyu40id2class_semseg[cur_cls])
...
def main():
    DATA_PATH = "" # Replace it with the path to scannet_frames_25k
    TARGET_DIR = "" # Replace it with the path to output path
    RGB_PATH = "./color"
    DEPTH_PATH = "./depth"
