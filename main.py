...
def make_args_parser():
    ...
    ##### Training Phase #####
    parser.add_argument(
        "--phase",  default='train_loc', type=str, choices=["train_loc", "train_dtcc", "finetune"]
    )
    ...
    # Used in Build img SetCriterion
...
def do_train(
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        ...
        # if epoch % args.eval_every_epoch == 9999 or epoch == (args.max_epoch - 1):
        if epoch % args.eval_every_epoch == 9999:
...
def test_model(args, model, model_no_ddp, criterion, dataset_config, dataloaders):
    ...
    for key,val in sd["model"].items():
        if "img_model" in key or "pc_model.mlp_heads.sem_cls_head.layers" in key:
            continue
        resume_var[key]=val
...
def main(local_rank, args):
    ...
    # Return "img_criterion" here
    model, _ = build_model(args, dataset_config)

    train_dtcc_only = True
    if args.phase == "train_dtcc":
        train_dtcc_only = True
    else:
        train_dtcc_only = False
